...
Netflix_df <- read_csv("data_storage/Netflix_Taiwan_Ranking.csv") # 讀取檔案
...
```{r}
Netflix_KR_Actor_df <- read_csv("data_storage/Netflix_KR_Actor_df.csv")
Netflix_KR_Actor_df %>%
  select(actor, actor_number) %>%
  filter(actor_number < 5) %>%
  count(actor) %>% arrange(desc(n))
  
Netflix_KR_Actor_df %>%
  select(TV, actor, actor_number) %>%
  filter(actor_number < 5) 

Netflix_KR_Actor_df %>%
  select(TV,movie_zh_name, actor, actor_number) 

Netflix_KR_Actor_df %>%
  select(movie_zh_name, actor) %>%
  filter(actor == "Song Kang")
